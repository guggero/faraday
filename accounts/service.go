...
const (
	...
	accountMiddlewareName = "faraday-account"
)
...
// passed directory.
func NewService(dir string, lnd lndclient.LndServices) (*Service, error) {
...
func (s *Service) Start(mainCtx context.Context) error {
	interceptErrChan, err := s.lnd.Client.RegisterRPCMiddleware(
		mainCtx, accountMiddlewareName, CondAccount,
		DefaultInterceptTimeout, func(ctx context.Context,
			request *lnrpc.RPCMiddlewareRequest) (
			*lnrpc.RPCMiddlewareResponse, error) {

			return s.intercept(ctx, request), nil
		},
	)
	if err != nil {
		return fmt.Errorf("error starting RPC interceptor: %v", err)
	}
	...
	go func() {
		...
		for {
			...
			case err := <-interceptErrChan:
				log.Errorf("Error in RPC interceptor: %v", err)
				return
